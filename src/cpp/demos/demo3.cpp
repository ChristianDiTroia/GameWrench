...
////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// Game status - change this from anywhere to determine if the player has won/lost
	bool win = false;
	bool gameOver = false;
	...
	////////// Using SFML to load and play sound effects //////////
	sf::Sound jumpSound;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	static void playerActions(AnimatedSprite& self) {
		...
		if (kb::isKeyPressed(kb::Space) && !inAir) {
			...
			if (jumpSound.getStatus() == sf::Sound::Status::Stopped) {
				jumpSound.play();
			}
		}
		if (inAir) {
	...
	static void enemyCollision(Sprite& sprite, Sprite& collidedWith, Vector2f collision) { 
		gameOver = true;
		defeatSound.play();
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	static SpriteCollection* room1Items() {
		...
		Effect& orange2 = *(new Effect(orange1));
		orange2.movePosition(meter.toPixels(3, 12));

		Effect& orange3 = *(new Effect(orange1));
		orange3.movePosition(meter.toPixels(0, -7));
	...
	// Room 2 collectables
	static SpriteCollection* room2Items() {
		Effect& orange1 = *(new Effect("./sprites/pixel_adventure_sprites/Items/Fruits/Orange.png", 32, 32));
		orange1.setAnimation(helpers::rowAnimation(0, 0, 16));
		orange1.playEffect(0.1);
		orange1.setPosition(meter.toPixels(20, 12));

		Effect& orange2 = *(new Effect(orange1));
		orange2.setPosition(meter.toPixels(3, 3));

		Effect& orange3 = *(new Effect(orange1));
		orange3.movePosition(meter.toPixels(10, -5));

		SpriteCollection* room2Items = new SpriteCollection;
		room2Items->addSprite(orange1)
			.addSprite(orange2)
			.addSprite(orange3);
...
void demos::runDemo3() {
	...
	pickupBuffer.loadFromFile("./sounds/pickupCoin.wav");
	pickupSound.setBuffer(pickupBuffer);

	sf::SoundBuffer defeatBuffer;
	defeatBuffer.loadFromFile("./sounds/explosion (1).wav");
	...
	BoxCollider room1ItemsCollider(itemCollision, false);
	room1ItemsCollider.applyCollision(ninjaFrog)
		.canCollideWith(*itemsRoom1);
	map.curRoom->addCollider(room1ItemsCollider);
	...
	BoxCollider room2ItemsCollider(itemCollision, false);
	...
	while (game.isPlaying()) {
		if (gameOver) { // player got hit by an enemy, reset game
			map.curToHead();
			ninjaFrog.setPosition(meter.toPixels(10, 13.5));
		...
		if (itemsCollected == 6 && !win) { // player wins
			winSound.play();
			win = true;
		}
	...
	deleteCollection(itemsRoom1);
	deleteCollection(itemsRoom2);
}